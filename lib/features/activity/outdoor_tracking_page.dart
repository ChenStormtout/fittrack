...
class _OutdoorTrackingPageState extends State<OutdoorTrackingPage>
  ...
  bool _followUser = true; // auto-center map ke posisi user
  ...
  Future<void> _startTracking() async {
    ...
    if (!success) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('GPS belum aktif atau izin lokasi ditolak')),
      );
  ...
  Color _activityColor(String type) {
    switch (type) {
      case 'Running': return const Color(0xFFE91E63);
      case 'Cycling': return const Color(0xFF2196F3);
      default:        return AppColors.primary;
    }
  }

  // Auto-center map saat dapat posisi baru
  void _onPositionUpdate(LatLng newPos) {
    if (_followUser) {
  ...
  Widget build(BuildContext context) {
    ...
    // Auto follow saat dapat posisi baru
    if (currentPos != null && _followUser && !isPaused) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) _onPositionUpdate(currentPos);
      });
    }
        ...
        children: [
          ...
          FlutterMap(
            ...
            options: MapOptions(
              ...
              initialZoom: 17,
              // Saat user geser peta manual → stop follow
            ...
            children: [
              ...
              if (routePoints.length > 1)
                PolylineLayer(
                  polylines: [
                    // Shadow polyline agar kontras di peta terang
                    Polyline(
                      points: routePoints,
                    ...
                    Polyline(
                      points: routePoints,
                      color: isPaused ? actColor.withOpacity(0.4) : actColor,
                      strokeWidth: 5,
                      strokeCap: StrokeCap.round,
                    ),
                  ],
                    ...
                    child: isPaused
                        ...
                        : AnimatedBuilder(
                            ...
                            builder: (_, __) => Container(
                              decoration: BoxDecoration(
                                shape: BoxShape.circle,
                                color: actColor.withOpacity(0.25 * _pulseAnim.value),
                              ),
                              child: Center(
                                child: Container(
                                  width: 20, height: 20,
                ...
                children: [
                  ...
                  AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                          color: (isPaused ? Colors.orange : actColor).withOpacity(0.3),
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      Icon(
                        isPaused ? Icons.pause_rounded : _activityIcon(widget.activityType),
                      ...
                      const SizedBox(width: 8),
                      AnimatedBuilder(
                        animation: isPaused ? _pauseAnim : const AlwaysStoppedAnimation(1.0),
                        builder: (_, __) => Container(
                          width: 8, height: 8,
                          decoration: BoxDecoration(
                            shape: BoxShape.circle,
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Icon(
                        ...
                        color: _followUser ? AppColors.primary : AppColors.textSecondary,
                        size: 20,
                      ),
                    ),
                  ),
          ...
          if (activityController.filteredPoints > 0)
            Positioned(
              ...
              child: Center(
                child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    const Icon(Icons.gps_not_fixed, color: Colors.white, size: 12),
                    const SizedBox(width: 4),
                    Text(
                      '${activityController.filteredPoints} titik GPS buruk difilter',
                    ...
                    if (isPaused)
                      Container(
                        ...
                        child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                          Icon(Icons.pause_circle_outline_rounded, color: Colors.orange, size: 18),
                          SizedBox(width: 8),
                          Text('Aktivitas sedang dijeda — Istirahatlah sejenak',
                          ...
                          children: [
                            Row(children: [
                              Expanded(child: _statCard(
                                icon: Icons.straighten_rounded, label: 'Jarak',
                                value: activityController.distanceKm.toStringAsFixed(2),
                                unit: 'km', color: actColor,
                              )),
                              const SizedBox(width: 12),
                              Expanded(child: _statCard(
                                icon: Icons.timer_outlined, label: 'Moving Time',
                                value: _formatDuration(activityController.durationSeconds),
                                unit: '', color: const Color(0xFF2196F3),
                              )),
                            ]),
                            const SizedBox(height: 10),
                            Row(children: [
                              Expanded(child: _statCard(
                                icon: Icons.terrain_rounded, label: 'Elevation',
                                value: activityController.elevationGainM.toStringAsFixed(0),
                                unit: 'm', color: const Color(0xFF4CAF50),
                              )),
                              const SizedBox(width: 12),
                              Expanded(child: _statCard(
                                icon: Icons.speed_rounded, label: 'Pace',
                                value: activityController.formattedPace,
                                unit: '/km', color: const Color(0xFFFF9800),
                              )),
                            ]),
                            if (_panelExpanded) ...[
                              const SizedBox(height: 10),
                              _statCard(
                                icon: Icons.local_fire_department_rounded, label: 'Kalori',
                                value: activityController.caloriesBurned.toStringAsFixed(0),
                                unit: 'kcal', color: const Color(0xFFE91E63),
                                fullWidth: true,
                              ),
                            ],
                          ],
                          ...
                          child: SizedBox(
                            ...
                            child: ElevatedButton.icon(
                              ...
                              style: ElevatedButton.styleFrom(
                                backgroundColor: isPaused ? const Color(0xFF4CAF50) : Colors.orange,
                                foregroundColor: Colors.white,
                                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)),
                                elevation: 0,
                              ),
                              icon: Icon(isPaused ? Icons.play_arrow_rounded : Icons.pause_rounded, size: 22),
      ...
      child: fullWidth
          ? Row(children: [
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('$value $unit', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: color)),
              ]),
            ])
          : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
              ...
              RichText(
                text: TextSpan(children: [
                  ...
                  if (unit.isNotEmpty)
                    TextSpan(text: ' $unit', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: color.withOpacity(0.7))),
                ]),
              ),
            ]),
    );
